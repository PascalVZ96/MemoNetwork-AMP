...
if [[ -f "$AMP_HTML" ]]; then
    sed -Ei '\#<script src="/Themes/AMPThemes/MemoNetwork/(DashboardPro|BuildInfo|MemoNetwork|SystemPolish|ControlCenterCollapse)\.js[^\"]*"></script>#d' "$AMP_HTML"
...
echo "Inklapbaar Control Center ingeschakeld."
